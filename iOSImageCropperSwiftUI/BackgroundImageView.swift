...
struct BackgroundImageView: View {
    ...
    var body: some View {
        ...
        .fullScreenCover(isPresented: $isCameraDisplay) {
            BackgroundImageMoveAndScale(imageType: .camera, croppedImage: $finalImage)
        }
        .statusBar(hidden: isImagePickerDisplay)
        .fullScreenCover(isPresented: $isImagePickerDisplay, onDismiss: loadImage) {
            BackgroundImageMoveAndScale(imageType: .gallery, croppedImage: $finalImage)
        }
    }
    func loadImage() {
